...
def addNew():
    ...
    with conn:
        cursor=conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS Student (Name TEXT,Email TEXT,Password TEXT,\
Gender TEXT,Programming TEXT)")
    count = cursor.execute("INSERT INTO Student (Name,Email,Password,Gender,Programming)\
VALUES(?,?,?,?,?)",(name,email,password,gender,prog))
...
def registerWindow():
    ...
    #The toplevel widget is used when a python application needs to represent some exstra
    #information, pop-up, or the group of widgets on window.
    ...
    Button(registerScreen,text="Submit",width=20,bg="blue",fg="white",pady=5,command=addNew).place(\
        x=180,y=380)
...
Button(root,text="Login Now",width=20,bg="blue",fg="white",pady=5,command=loginNow).place(x=170,y=280)

Button(root,text="Don't Have  Account! Create one",bg="red",fg="white",font=("bold",10),\
       command = registerWindow).place(x=170,y=230)    
